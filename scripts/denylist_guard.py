...
tokens from every file in the working tree, from the full git history
...
Candidate extraction, on lowercased text:

1. every maximal run of ``[a-z0-9_.:@/+-]`` (identifier-like tokens, so
   hyphenated aliases and ``prefix:name`` forms match whole);
2. every maximal run of ``[a-z0-9]`` (bare words);
3. every substring of length 3..12 of each bare word, so a short term embedded
   in a longer identifier (``termhost``, ``term_router``) still matches.
...
def candidates(text: str) -> Iterator[str]:
    ...
    for m in WORD_RE.finditer(lower):
        ...
        if word not in seen:
            ...
            yield word
        n = len(word)
        for length in range(MIN_SUB, min(MAX_SUB, n - 1) + 1):
