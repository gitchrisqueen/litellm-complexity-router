...
**The circularity problem, and the four controls.** The scorer is a keyword counter over three regex lists (code, technical, multi-step), two plain word lists (reasoning, simple) and one wh-word regex; any rubric written after reading the source shares its intuitions, so accuracy against such a set is a fit statistic. Four controls break the loop, and this README states which are in place:
